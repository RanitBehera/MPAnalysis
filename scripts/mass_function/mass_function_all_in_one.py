...
CBOX        = galspec.NavigationRoot(L50N640)
COSMOLOGY   = CBOX.GetCosmology("MassFunctionLitrature")
SNAP        = CBOX.RSG(SNAP_NUM)
...
# Box mass function
def PlotBMF(M,dn_dlogM,error,min_mass,right_skip_count,include_deviation,color,leg):
    # Filters
    ...
    #plot
    ax[0].plot(M,dn_dlogM,'-',label= BOX_TEXT + leg,lw=2,color=color)
...
for i,PLOT in enumerate(CURVE_LIST):
    ...
    if FOF in PLT_HALO:
        ...
        if DM in PLT_TYPE:
            ...
            PlotBMF(M,dn_dlogM,error,HALO_DEF * MASS_TABLE[1] * MASS_UNIT,RIGHT_SKIP[i],True,COLORS1[i]," (FoF)")
        if GAS in PLT_TYPE:
            M, dn_dlogM,error = MassFunction(M_GAS,BOX_SIZE,BIN_SIZE)
            PlotBMF(M,dn_dlogM,error,HALO_DEF * MASS_TABLE[0] * MASS_UNIT,RIGHT_SKIP[i],False,COLORS1[i]," (FoF)")
        if STAR in PLT_TYPE:
            M, dn_dlogM,error = MassFunction(M_STAR,BOX_SIZE,BIN_SIZE)
            PlotBMF(M,dn_dlogM,error,HALO_DEF * MASS_TABLE[4] * MASS_UNIT,RIGHT_SKIP[i],False,COLORS1[i]," (FoF)")
    ...
    if ROCKSTAR in PLT_HALO:
        ...
        if DM in PLT_TYPE:
            M, dn_dlogM,error = MassFunction(M_DM,BOX_SIZE,BIN_SIZE)
            PlotBMF(M,dn_dlogM,error,HALO_DEF * MASS_TABLE[1] * MASS_UNIT,RIGHT_SKIP[i],False,COLORS1[i]," (Virial)")
        if GAS in PLT_TYPE:
            M, dn_dlogM,error = MassFunction(M_GAS,BOX_SIZE,BIN_SIZE)
            PlotBMF(M,dn_dlogM,error,HALO_DEF * MASS_TABLE[0] * MASS_UNIT,RIGHT_SKIP[i],False,COLORS1[i]," (Virial)")
        if STAR in PLT_TYPE:
            M, dn_dlogM,error = MassFunction(M_STAR,BOX_SIZE,BIN_SIZE)
            PlotBMF(M,dn_dlogM,error,HALO_DEF * MASS_TABLE[4] * MASS_UNIT,RIGHT_SKIP[i],False,COLORS1[i]," (FoF)")
...
ax[0].set_xscale('log')
ax[0].set_yscale('log')
ax[0].legend(title="Dark Matter HMF")
ax[0].set_ylabel("$dn/d\log(M/M_{\odot})$")
ax[0].grid(alpha=0.3)
ax[1].set_xlim(left=5*10**7)
...
plt.suptitle(f"HALO MASS FUNCTION\nz={numpy.round(REDSHIFT,2)}")
